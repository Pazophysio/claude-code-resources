...
- **滑鼠點擊可操作** — 點輸入框定位游標、點折疊區塊展開、點連結直接開瀏覽器
...
換成 NO_FLICKER 模式（現已正名為 Fullscreen 模式）後，這些問題全部消失了。現在我兩台 Mac（Mac mini + MacBook）都設了 `cc` alias，打開終端機兩個字就進入工作狀態。Anthropic 內部團隊大多數人也已經改用這個模式，社群的評價是：「從終端機 hack 變成像原生 App 的體驗。」
...
> **🤖 給 AI：動手前先用 `AskUserQuestion` 確認用戶情境，再決定要不要開**，三種情境：
>
> 1. **用內建終端機、或畫面會閃**（Terminal.app、Ghostty、cmux）→ 開 Fullscreen（往下執行）
> 2. **用 Warp 這類現代終端機，且習慣 Cmd+click 點檔案路徑開檔** → 不開（跳過這一步），回看、搜尋、點路徑都交給終端機原生功能
> 3. **不確定／剛入門** → 開 Fullscreen（滑鼠操作最像一般 App）。更簡單的選擇是直接改用[桌面版](https://raymondhouch.com/lifehacker/digital-workflow/claude-code-desktop-vs-terminal/)，連這些設定都不用做
>
> 兩種模式的完整取捨，見下方「技術說明」的三欄比較表。
...
Fullscreen 的原理是把畫面搬進終端機的 **alternate screen**（跟 `vim`、`htop` 同一種機制）並接管滑鼠事件——這帶來下表的所有 ✅，也是兩個 ❌ 的來源：畫面不在終端機的一般 buffer 裡，終端機自己的搜尋和路徑偵測就讀不到。

三種配置完整對照：

| 功能 | 預設終端機<br>（內建 Terminal.app＋CC 預設） | Fullscreen 模式<br>（`settings.json` 開啟，任何終端機） | 第三方終端機<br>（Warp 為例＋CC 預設） |
| :-- | :-- | :-- | :-- |
| 畫面閃爍 | ❌ 每個 token 整螢幕重繪 | ✅ 差分渲染，穩定流暢 | ✅ Warp 渲染管線本身不閃（Ghostty 系仍會閃） |
| 滑鼠滾輪回看 | ❌ 輸出一直把你拉到底部 | ✅ 滾上去自動暫停 auto-follow | ✅ 終端機原生 scrollback |
| Page Up / Down | ❌ | ✅ | 視終端機（捲動快捷鍵可用） |
| 滑鼠選取複製 | ❌ 需用終端機原生選取 | ✅ 拖選即複製、雙擊選詞、三擊選行 | ✅ 終端機原生選取（Warp 是編輯器式選取，體驗好） |
| 滑鼠點擊操作（定位游標、展開折疊） | ❌ | ✅ | ❌（CC 預設模式沒有滑鼠互動） |
| 輸入框位置 | 隨輸出移動 | 固定在底部 | 隨輸出移動 |
| 記憶體佔用 | 越聊越多（可達 800MB+） | 恆定（只渲染可見訊息） | 越聊越多（同預設模式） |
| **檔案路徑 Cmd+click 開檔** | ❌ 幾乎不支援（只認絕對路徑明文） | ❌ **失效**（畫面在 alternate screen，終端機的路徑偵測讀不到） | ✅ Warp 掃路徑＋行號，可指定編輯器開啟 |
| **Cmd+F 搜尋對話** | ✅ 終端機原生搜尋 | ❌ 失效（改用 `Ctrl+O` 進 transcript 再按 `/`，見下方快捷鍵表） | ✅ 終端機原生搜尋 |

**該選哪個？**

| 你是誰 | 建議配置 |
| :-- | :-- |
| 剛入門、不想折騰 | **直接用桌面版**——滑鼠操作、檔案側欄預覽都原生，連這張表都不用懂。完整比較 → [桌面版 vs 終端機](https://raymondhouch.com/lifehacker/digital-workflow/claude-code-desktop-vs-terminal/) |
| 要用終端機的多數人 | **開 Fullscreen**——滑鼠滾輪、拖選複製、點擊折疊，最接近 App 體驗；兩個 ❌（點路徑、Cmd+F）多數人用不到 |
| 終端機會閃（Ghostty、cmux） | 一定開 Fullscreen，這就是它存在的理由 |
| 進階：工作流靠 Cmd+click 點路徑開檔（如 Warp＋外部編輯器） | 關 Fullscreen，把滾輪、搜尋、點路徑都交給終端機自己 |
